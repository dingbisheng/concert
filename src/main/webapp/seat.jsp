                ...
                <c:choose>
                    <c:when test="${seat.seatState==0}">
                        ...
                        <img id="${seat.seatRow},${seat.seatCol}" title="${seat.seatRow}排${seat.seatCol}座  ${seat.seatPrice}元   可售"
                             ...
                             onclick="doClickSeat(${seat.seatRow},${seat.seatCol},${seat.seatId},${seat.seatPrice})" class="selectSeat"/>
                    </c:when>

                    <c:when test="${seat.seatState==-2}">
                        <%--座位被锁定--%>
                        <img title="${seat.seatRow}排${seat.seatCol}座  ${seat.seatPrice}元   不可售" src="<%=basePath %>/concert/picture/l.png" width="20" height="20" class="selectSeat"/>
                    </c:when>

                    <c:when test="${seat.seatState==-1}">
                        <%--没有座位--%>
                        <img title="${seat.seatRow}排${seat.seatCol}座  ${seat.seatPrice}元   不可售" src="<%=basePath %>/concert/picture/n.png" width="20" height="20" class="selectSeat"/>
                    </c:when>

                    <c:otherwise>
                        <%--座位被购买--%>
                        <img title="${seat.seatRow}排${seat.seatCol}座  ${seat.seatPrice}元   不可售" src="<%=basePath %>/concert/picture/m.png" width="20" height="20" class="selectSeat"/>
                    </c:otherwise>
...
<script type="text/javascript">
    ...
    function doClickSeat(row,col,seatId,seatPrice) {
